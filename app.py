...
import json
from dataio.loaders import load_sf_crime_latest, RESULTS_DIR
from components.last_update import get_last_update

st.set_page_config(page_title="SF Crime Monitor", layout="wide")
st.title("SF Crime Monitor")
...
if info:
    c4.metric("Tarih Aralığı", f"{info['date_min'] or '?'} → {info['date_max'] or '?'}")
else:
...
mask = (df["event_hour"].between(hour[0], hour[1])) if "event_hour" in df.columns else True
if geoid_prefix and "GEOID" in df.columns:
    mask = mask & df["GEOID"].astype(str).str.startswith(geoid_prefix)
...
if all(c in dfv.columns for c in ["lat","lon"]):
    st.write("### Nokta Haritası (örnek)")
    st.map(dfv[["lat","lon"]].dropna().sample(min(5000, len(dfv))), size=10)
else:
    st.info("Harita için lat/lon bulunamadı.")

# alt bilgi
if info:
    st.caption(f"Last update: {info['when']} • rows={info['rows']:,} • src={info['source']}")
 ... benim elimde kod vbardı zaten, ufak tefek hataalr olsa de su şekilde isid.. from __future__ import annotations
